...
class Layer_04_uno(nn.Module):
    def __init__(self, *input_shapes, **kwargs):
        super(Layer_04_uno, self).__init__(**kwargs)
...
#        _optimizer = optim.SGD(self.parameters(), lr=0.001, momentum=0.2)
        ...
        if validation_data is not None:
            ...
            if isinstance(x_test, (np.ndarray, list, tuple)):
...
#                _, yb = torch.max(yb, dim = 1)
        ...
        for epoch in range(epochs):
            ...
            print("[", end='')
            losses, nums = zip(
                *(loss_batch(self._criterion, xb, yb, self._optimizer) for xb , yb in train_dl)
            )
            print("]", end='')
            ...
            if validation_data is not None:
                ...
                with torch.no_grad():
                    losses, nums = zip(
                        *(loss_batch(self._criterion, xb, yb, self._optimizer) for xb , yb in valid_dl)
                    )
                print("]", end='')
